...
from fastPose.processing.image_processing import tensor_vstack
...
class AnchorLoader(mx.io.DataIter):
    def __init__(self, feat_sym, roidb, batch_size=1, shuffle=False, ctx=None, work_load_list=None,
                 feat_stride=16, anchor_scales=(4, 8, 16), anchor_rotates=(0,), allowed_border=0):
        """
        ...
        self.anchor_rotates = anchor_rotates
        ...
        if config.TRAIN.END2END:
            self.data_name = ['data', 'im_info', 'gt_poses']
        else:
            self.data_name = ['data'] 
        self.label_name = ['label', 'pose_target', 'pose_weight']
    ...
    def infer_shape(self, max_data_shape=None, max_label_shape=None):
        ...
        label = minibatch.assign_anchor(feat_shape[0], np.zeros((0, 5)), im_info,
                                        self.feat_stride, self.anchor_scales, self.anchor_rotates, self.allowed_border)
    ...
    def get_batch(self):
        ...
        for data, label in zip(data_list, label_list):
            ...
            # add gt_poses to data for e2e
            data['gt_poses'] = label['gt_poses'][np.newaxis, :, :]

            # assign anchor for label
            label = minibatch.assign_anchor(feat_shape, label['gt_poses'], data['im_info'],
                                            self.feat_stride, self.anchor_scales,
                                            self.anchor_rotates, self.allowed_border)
